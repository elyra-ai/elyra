(self["webpackChunk_jupyterlab_application_top"] = self["webpackChunk_jupyterlab_application_top"] || []).push([[9046,5606],{
...
var map = __webpack_require__(61662);
...
const computeParams = () => {
  if (params === undefined) {
    if (isNode) {
      ...
      params = map/* create */.vt(); // eslint-disable-next-line no-undef
...
var lib0_set = __webpack_require__(25404);
 ...
 * @return {string}
 */
const mapToStyleString = m => map/* map */.Tj(m, (value, key) => `${key}:${value};`).join('')
...
const computeBrowserLoggingArgs = (args) => {
  const strBuilder = []
  const styles = []
  const currentStyle = map/* create */.vt()
  /**
...
var object = __webpack_require__(70641);
    ...
    dss[dssI].clients.forEach((delsLeft, client) => {
      if (!merged.clients.has(client)) {
        ...
        for (let i = dssI + 1; i < dss.length; i++) {
          array/* appendTo */.S2(dels, dss[i].clients.get(client) || []);
        }
...
const addToDeleteSet = (ds, client, clock, length) => {
  map/* setIfUndefined */._4(ds.clients, client, () => []).push(new DeleteItem(clock, length));
};
...
const writeDeleteSet = (encoder, ds) => {
  ...
  array/* from */.HT(ds.clients.entries())
    .sort((a, b) => b[0] - a[0])
...
const readDeleteSet = decoder => {
  ...
  for (let i = 0; i < numClients; i++) {
    ...
    if (numberOfDeletes > 0) {
      const dsField = map/* setIfUndefined */._4(ds.clients, client, () => []);
...
class Doc extends observable/* Observable */.c {
  ...
  getSubdocGuids () {
    return new Set(array/* from */.HT(this.subdocs).map(doc => doc.guid))
  }
  ...
  get (name, TypeConstructor = AbstractType) {
    const type = map/* setIfUndefined */._4(this.share, name, () => {
      // @ts-ignore
   ...
   */
  destroy () {
    array/* from */.HT(this.subdocs).forEach(subdoc => subdoc.destroy());
    const item = this._item;
    if (item !== null) {
      this._item = null;
...
const writeClientsStructs = (encoder, store, _sm) => {
  ...
  array/* from */.HT(sm.entries()).sort((a, b) => b[0] - a[0]).forEach(([client, clock]) => {
    // @ts-ignore
...
const readClientsStructRefs = (decoder, doc) => {
   ...
   */
  const clientRefs = map/* create */.vt();
...
const integrateStructs = (transaction, store, clientsStructRefs) => {
  ...
  let clientsStructRefsIds = array/* from */.HT(clientsStructRefs.keys()).sort((a, b) => a - b);
  ...
  while (true) {
    if (stackHead.constructor !== Skip) {
      const localClock = map/* setIfUndefined */._4(state, stackHead.id.client, () => getState(store, stackHead.id.client));
...
const writeStateVector = (encoder, sv) => {
  ...
  array/* from */.HT(sv.entries()).sort((a, b) => b[0] - a[0]).forEach(([client, clock]) => {
...
const splitSnapshotAffectedStructs = (transaction, snapshot) => {
  const meta = map/* setIfUndefined */._4(transaction.meta, splitSnapshotAffectedStructs, lib0_set/* create */.vt);
...
const writeUpdateMessageFromTransaction = (encoder, transaction) => {
  if (transaction.deleteSet.clients.size === 0 && !map/* any */.bz(transaction.afterState, (clock, client) => transaction.beforeState.get(client) !== clock)) {
    return false
  }
...
const addChangedTypeToTransaction = (transaction, type, parentSub) => {
  ...
  if (item === null || (item.id.clock < (transaction.beforeState.get(item.id.client) || 0) && !item.deleted)) {
    map/* setIfUndefined */._4(transaction.changed, type, lib0_set/* create */.vt).add(parentSub);
  }
};
...
class UndoManager extends observable/* Observable */.c {
  ...
  constructor (typeScope, {
    ...
    doc = /** @type {Doc} */ (array/* isArray */.cy(typeScope) ? typeScope[0].doc : typeScope.doc)
  } = {}) {
    super();
    /**
   ...
   */
  addToScope (ytypes) {
    ytypes = array/* isArray */.cy(ytypes) ? ytypes : [ytypes];
          ...
          if (this.adds(item)) {
            ...
            if (this.deletes(item)) {
              if (prev !== null && this.deletes(prev)) {
                action = 'delete';
                oldValue = array/* last */.HV(prev.content.getContent());
              } else {
                return
              }
            } else {
            ...
            if (this.deletes(item)) {
              action = 'delete';
              oldValue = array/* last */.HV(/** @type {Item} */ item.content.getContent());
            } else {
              return // nop
            }
          }
...
const callTypeObservers = (type, transaction, event) => {
  ...
  while (true) {
    // @ts-ignore
    map/* setIfUndefined */._4(changedParentTypes, type, () => []).push(event);
    if (type._item === null) {
      break
    }
...
const cleanupFormattingGap = (transaction, start, curr, startAttributes, currAttributes) => {
   ...
   */
  const endFormats = map/* create */.vt();
...
const cleanupYTextFormatting = type => {
  ...
  transact(/** @type {Doc} */ (type.doc), transaction => {
    ...
    let end = type._start;
    let startAttributes = map/* create */.vt();
    const currentAttributes = map/* copy */.C(startAttributes);
    while (end) {
      if (end.deleted === false) {
        switch (end.content.constructor) {
          ...
          default:
            ...
            startAttributes = map/* copy */.C(currentAttributes);
            start = end;
            break
        }
      }
...
const deleteText = (transaction, currPos, length) => {
  const startLength = length;
  const startAttrs = map/* copy */.C(currPos.currentAttributes);
...
class YXmlFragment extends AbstractType {
  ...
  querySelectorAll (query) {
    query = query.toUpperCase();
    // @ts-ignore
    return array/* from */.HT(new YXmlTreeWalker(this, element => element.nodeName && element.nodeName.toUpperCase() === query))
  }
