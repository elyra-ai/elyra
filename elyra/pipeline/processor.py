...
class RuntimePipelineProcessor(PipelineProcessor):  # ABC
    ...
    def _get_runtime_images(self):
        """
        Retrieve associated runtime configuration based on processor type
        :return: metadata in json format
        """
        try:
            runtime_images = MetadataManager(namespace=MetadataManager.NAMESPACE_RUNTIME_IMAGES).get_all()
            return runtime_images
        except BaseException as err:
            self.log.error('Error retrieving runtime images', exc_info=True)
            raise RuntimeError('Error retrieving runtime images', err) from err
