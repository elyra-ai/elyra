...
def test_fail_without_name():
    with pytest.raises(TypeError):
        ExecuteFileOp(pipeline_name="test-pipeline",
                      experiment_name="experiment-name",
                      notebook="test_notebook.ipynb",
                      cos_endpoint="http://testserver:32525",
                      cos_bucket="test_bucket",
...
def test_properly_set_notebook_name_when_in_subdirectory():
    notebook_op = ExecuteFileOp(name="test",
                                pipeline_name="test-pipeline",
                                experiment_name="experiment-name",
...
def test_normalize_label_value():
    ...
    # test max length (63)
    assert ExecuteFileOp._normalize_label_value('a' * 63) ==\
        'a' * 63
    assert ExecuteFileOp._normalize_label_value('a' * 64) ==\
        'a' * 63  # truncated
    ...
    for c in string.printable:
        if c in string.ascii_letters + string.digits:
            ...
            # max length
            assert ExecuteFileOp._normalize_label_value('b' * 62 + c) == \
                   ('b' * 62 + c)
            # max length exceeded
            assert ExecuteFileOp._normalize_label_value('b' * 63 + c) == \
                   ('b' * 63)
        else:
    ...
    for c in string.printable:
        if c in string.ascii_letters + string.digits:
            ...
            assert ExecuteFileOp._normalize_label_value(c + 'b' + c) ==\
                'a' + rv + 'b' + rv + 'a'
            # max length
            assert ExecuteFileOp._normalize_label_value(c + 'b' * 59 + c) == \
                   ('a' + rv + 'b' * 59 + rv + 'a')
            # max length exceeded after processing, scenario 1
            # resolved by adding char before first, replace last
            assert ExecuteFileOp._normalize_label_value(c + 'b' * 60 + c) == \
                   ('a' + rv + 'b' * 60 + 'a')
            ...
            assert ExecuteFileOp._normalize_label_value(c + 'b' * 60 + c * 3) == \
                   ('a' + rv + 'b' * 60 + 'a')
    ...
    for c in string.printable:
        if c in string.ascii_letters + string.digits + '-_.':
            assert ExecuteFileOp._normalize_label_value('A' + c + 'Z') ==\
                'A' + c + 'Z'
        else:
