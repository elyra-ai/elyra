...
async def test_bogus_namespace(fetch, bogus_location):
    # Validate missing is not found.  Remove the bogus location to ensure its not created
    shutil.rmtree(bogus_location)
    with pytest.raises(HTTPClientError) as e:
        await fetch('elyra', 'metadata', 'bogus', 'missing')

    assert expected_http_error(e, 400)
    assert not os.path.exists(bogus_location)


async def test_missing_instance(fetch, setup_data):
    # Validate missing is not found
    name = 'missing'
    with pytest.raises(HTTPClientError) as e:
        await fetch('elyra', 'metadata', METADATA_TEST_NAMESPACE, name)
    assert expected_http_error(e, 404)


async def test_invalid_instance(fetch, setup_data):
    # Validate invalid throws 404 with validation message
    with pytest.raises(HTTPClientError) as e:
        await fetch('elyra', 'metadata', METADATA_TEST_NAMESPACE, 'invalid')
    assert expected_http_error(e, 400)


async def test_valid_instance(fetch, setup_data):
    # Ensure valid metadata can be found
    r = await fetch('elyra', 'metadata', METADATA_TEST_NAMESPACE, 'valid')
...
async def test_get_instances(fetch, setup_data):
    # Ensure all valid metadata can be found
    r = await fetch('elyra', 'metadata', METADATA_TEST_NAMESPACE)
    assert r.code == 200
    metadata = json.loads(r.body.decode())
...
async def test_get_empty_namespace_instances(fetch, namespace_location, setup_data):
...
async def test_get_hierarchy_instances(fetch, setup_hierarchy):
    # Ensure all valid metadata can be found
    r = await fetch('elyra', 'metadata', METADATA_TEST_NAMESPACE)
    assert r.code == 200
    metadata = json.loads(r.body.decode())
...
async def test_create_instance(fetch):
    ...
    r = await fetch('elyra', 'metadata', METADATA_TEST_NAMESPACE, body=body, method='POST')
    assert r.code == 201
...
async def test_create_hierarchy_instance(fetch, setup_hierarchy):
    ...
    with pytest.raises(HTTPClientError) as e:
        await fetch('elyra', 'metadata', METADATA_TEST_NAMESPACE, body=body, method='POST')
...
async def test_create_invalid_instance(fetch):
    """Create a simple instance - not conflicting with factory instances. """
...
async def test_create_instance_missing_schema(fetch, namespace_location):
    ...
    with pytest.raises(HTTPClientError) as e:
        await fetch('elyra', 'metadata', METADATA_TEST_NAMESPACE, 'missing_schema')
    assert expected_http_error(e, 404)


async def test_update_non_existent(fetch, namespace_location):
    ...
    with pytest.raises(HTTPClientError) as e:
        await fetch('elyra', 'metadata', METADATA_TEST_NAMESPACE, 'valid', body=body, method='PUT')
    assert expected_http_error(e, 404)


async def test_update_instance(fetch, namespace_location):
    ...
    # Update instance
    r = await fetch('elyra', 'metadata', METADATA_TEST_NAMESPACE, 'valid', body=body, method='PUT')
    assert r.code == 200
...
async def test_invalid_update(fetch, namespace_location):
    ...
    r = await fetch('elyra', 'metadata', METADATA_TEST_NAMESPACE, 'update_bad_md')
    assert r.code == 200
    instance = json.loads(r.body.decode())
    ...
    with pytest.raises(HTTPClientError) as e:
        await fetch('elyra', 'metadata', METADATA_TEST_NAMESPACE, 'update_bad_md', body=body2, method='PUT')
    assert expected_http_error(e, 400)
...
async def test_update_fields(fetch, namespace_location):
    ...
    r = await fetch('elyra', 'metadata', METADATA_TEST_NAMESPACE, 'update_fields', body=body, method='PUT')
    assert r.code == 200
    instance = json.loads(r.body.decode())
    assert instance['metadata']['number_range_test'] == 7
...
async def test_update_hierarchy_instance(fetch, setup_hierarchy):
    ...
    r = await fetch('elyra', 'metadata', METADATA_TEST_NAMESPACE, 'byo_2', body=body, method='PUT')
    assert r.code == 200
    ...
    with pytest.raises(HTTPClientError) as e:
        await fetch('elyra', 'metadata', METADATA_TEST_NAMESPACE, 'byo_2', body=body, method='PUT')
    assert expected_http_error(e, 400)

    # Confirm no update occurred
    r = await fetch('elyra', 'metadata', METADATA_TEST_NAMESPACE, 'byo_2')
    assert r.code == 200
    instance = json.loads(r.body.decode())
    assert instance['name'] == 'byo_2'


async def test_delete_instance(fetch, namespace_location, setup_data):
    ...
    with pytest.raises(HTTPClientError) as e:
        await fetch('elyra', 'metadata', METADATA_TEST_NAMESPACE, 'missing', method='DELETE')
    assert expected_http_error(e, 404)

    create_json_file(namespace_location, 'valid.json', valid_metadata_json)

    r = await fetch('elyra', 'metadata', METADATA_TEST_NAMESPACE, 'valid', method='DELETE')
    assert r.code == 204

    # Confirm deletion
    with pytest.raises(HTTPClientError) as e:
        await fetch('elyra', 'metadata', METADATA_TEST_NAMESPACE, 'valid', method='DELETE')
    assert expected_http_error(e, 404)


async def test_delete_hierarchy_instance(fetch, namespace_location, setup_hierarchy):
    """Create a simple instance - that conflicts with factory instances and delete it only if local. """

    with pytest.raises(HTTPClientError) as e:
        await fetch('elyra', 'metadata', METADATA_TEST_NAMESPACE, 'byo_2', method='DELETE')
    ...
    r = await fetch('elyra', 'metadata', METADATA_TEST_NAMESPACE, 'byo_2', method='DELETE')
    assert r.code == 204


async def test_bogus_schema(fetch):
    # Validate missing is not found

    # Remove self.request (and other 'self.' prefixes) once transition to jupyter_server occurs
    with pytest.raises(HTTPClientError) as e:
        await fetch('elyra', 'schema', 'bogus')
    assert expected_http_error(e, 404)


async def test_missing_runtimes_schema(fetch):
    # Validate missing is not found
    with pytest.raises(HTTPClientError) as e:
        await fetch('elyra', 'schema', 'runtimes', 'missing')
    assert expected_http_error(e, 404)


async def test_get_runtimes_schemas(fetch):
    # Ensure all schema for runtimes can be found
    await _get_namespace_schemas(fetch, 'runtimes', ['kfp'])


async def test_get_code_snippets_schemas(fetch):
    # Ensure all schema for code-snippets can be found
    await _get_namespace_schemas(fetch, 'code-snippets', ['code-snippet'])


async def test_get_test_schemas(fetch):
    # Ensure all schema for metadata_tests can be found
    await _get_namespace_schemas(fetch, METADATA_TEST_NAMESPACE, ['metadata-test', 'metadata-test2'])


async def test_get_runtimes_schema(fetch):
    # Ensure all schema for runtimes can be found
    await _get_namespace_schema(fetch, 'runtimes', 'kfp')


async def test_get_code_snippets_schema(fetch):
    # Ensure all schema for code-snippets can be found
    await _get_namespace_schema(fetch, 'code-snippets', 'code-snippet')


async def test_get_test_schema(fetch):
    # Ensure all schema for code-snippets can be found
    await _get_namespace_schema(fetch, METADATA_TEST_NAMESPACE, 'metadata-test')


async def _get_namespace_schemas(fetch, namespace, expected):
    r = await fetch('elyra', 'schema', namespace,)
    assert r.code == 200
...
async def _get_namespace_schema(fetch, namespace, expected):
    r = await fetch('elyra', 'schema', namespace, expected)
    assert r.code == 200
...
async def test_get_namespaces(fetch):
    expected_namespaces = ['runtimes', 'code-snippets']

    r = await fetch('elyra', 'namespace')
    assert r.code == 200
