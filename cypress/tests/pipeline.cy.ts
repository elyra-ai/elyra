...
describe('Pipeline Editor tests', () => {
  ...
  afterEach(() => {
    ...
    // delete runtime configurations and example catalogs
    cy.exec(
      'elyra-metadata remove runtimes --name=kfp_test_runtime 2>/dev/null; ' +
        'elyra-metadata remove runtimes --name=airflow_test_runtime 2>/dev/null; ' +
        'elyra-metadata remove component-catalogs --name=example_components 2>/dev/null',
