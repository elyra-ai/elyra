...
- A Kubernetes Cluster without Apache Airflow installed
    ...
    - Helm v3.0 or later
    - Use the [Helm chart](https://github.com/airflow-helm/charts/tree/v4.0.0/charts/airflow) available in the Airflow source distribution with the [Elyra sample configuration](https://raw.githubusercontent.com/elyra-ai/elyra/v4.0.0/etc/kubernetes/airflow/helm/values.yaml).
...
Take note of the following information:
 ...
 - Repository branch name (e.g. `v4.0.0`)
...
2. Download, review, and customize the [sample `helm` configuration](https://raw.githubusercontent.com/elyra-ai/elyra/v4.0.0/etc/kubernetes/airflow/helm/values.yaml) (or customize an existing configuration). This sample configuration will use the `KubernetesExecutor` by default.
   ...
   - Set `git.ref` to the DAG branch, e.g. `v4.0.0` you created earlier.
   ...
   The container image is created using [this `Dockerfile`](https://github.com/elyra-ai/elyra/tree/v4.0.0/etc/docker/airflow) and published on [Docker Hub](https://hub.docker.com/r/elyra/airflow) and [quay.io](https://quay.io/repository/elyra/airflow).
